...
function Assert-DeclaredModelArtifacts(
    ...
    if ($null -ne $unifiedDeclarationProperty -and $null -ne $unifiedDeclarationProperty.Value) {
        ...
        if ([int]$UnifiedEvidence.ArchitectureVersion -eq 13) {
            ...
            # Keep accepting those packages, while rejecting v13-only policy
            # fields or a conflicting version when the optional field exists.
            $versionedUnifiedBindingInvalid = (
...
function Assert-ResultProvenanceAndPolicy(
    ...
    if ([int]$UnifiedEvidence.ArchitectureVersion -eq 13) {
        ...
        if ($null -eq $statusProperty -or $null -eq $statusProperty.Value) {
            throw "Legacy v13 result has no transfer_status field object: $ResultPath"
        }
        ...
        if ($null -eq $policyProperty `
            ...
            -or $null -eq $stateProperty) {
            throw "Legacy v13 result transfer_status has incomplete review-only policy evidence: $ResultPath"
        }
        if ([string]$stateProperty.Value -eq "absent") {
            throw "Legacy v13 result transfer_status is absent; the complete delivery path requires visible OCR text: $ResultPath"
        }
        else {
            ...
            if ([string]::IsNullOrWhiteSpace($rawStatus) `
                ...
                throw "Legacy v13 result transfer_status has incomplete or inconsistent OCR text evidence: $ResultPath"
            }
            if ([string]$stateProperty.Value -ne "review" `
                ...
                throw "Legacy v13 result transfer_status escaped the review-only policy: $ResultPath"
            }
        }
    }
}
...
try {
    ...
    $unifiedArtifactDeclaration = [ordered]@{
        kind = [string]$stagingSourceEvidence.Unified.Kind
        architecture_version = [int]$stagingSourceEvidence.Unified.ArchitectureVersion
    ...
    if ([int]$stagingSourceEvidence.Unified.ArchitectureVersion -eq 13) {
        $unifiedArtifactDeclaration["status_text_delivery_policy"] = `
            [string]$stagingSourceEvidence.Unified.StatusTextDeliveryPolicy
        $unifiedArtifactDeclaration["status_text_review_value"] = `
            [string]$stagingSourceEvidence.Unified.StatusTextReviewValue
    }
    $modelArtifactDeclaration = [ordered]@{
        ...
        unified_ocr = $unifiedArtifactDeclaration
    }
